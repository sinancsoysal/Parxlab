...
namespace Parxlab.Common.Extensions
{
    ...
    {
        public static bool MatchIbanRule(string arg)
        {
            var rule1 = arg.StartsWith("TR", StringComparison.InvariantCultureIgnoreCase);
            var rule2 = IsDigitsOnly(arg.Replace("TR", ""));
            return rule1 && rule2;
        }

        public static bool MatchPhoneNumberRule(string arg)
        {
            var rule1 = arg.StartsWith("0");
            var rule2 = IsDigitsOnly(arg);
            return rule1 && rule2;
        }
